...
static void treeFactory(GenericTree<int>& tree) {
  ...
  // ...
  GenericTree<int>::TreeNode* rootPtr = tree.getRootPtr();
  rootPtr -> data = 4;

  GenericTree<int>::TreeNode* leftPtr = rootPtr->addChild(8);
  GenericTree<int>::TreeNode* rightPtr = rootPtr->addChild(15);

  GenericTree<int>::TreeNode* leftLeftPtr = leftPtr->addChild(16);
  GenericTree<int>::TreeNode* leftRightPtr = leftPtr->addChild(23);

  GenericTree<int>::TreeNode* leftLeftLeftPtr = leftLeftPtr->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while (!nodesToExplore.empty()) {
    TreeNode* frontNodePtr = nodesToExplore.front();
    nodesToExplore.pop();
    results.push_back(frontNodePtr -> data);

    for (auto childPtr : frontNodePtr -> childrenPtrs) {
      nodesToExplore.push(childPtr);
    }
  }
